...
public class ProofProcessor {
    ...
    public static String tptpProof(List<ProofStep> proofSteps) {
        ...
        try {
            for (int j = 0; j < proofSteps.size(); j++) {
                ProofStep step = proofSteps.get(j);
                boolean isLeaf = step.premises.isEmpty() ||
                        (step.premises.size() == 1 && step.premises.get(0).intValue() == 0);
    ...
    public boolean equalsAnswer(int answerNum, String expectedAnswer) {
        ...
        if (answer.attributes.get("result").equalsIgnoreCase("no"))
            return false;
        if (answer.attributes.get("result").equalsIgnoreCase("yes") &&
        ...
        if (bindingSet != null) {
            ...
            for (int j = 0; j < binding.subelements.size(); j++) {
                BasicXMLelement variableBinding = binding.subelements.get(j);
                String variable = variableBinding.attributes.get("name");
                String value = variableBinding.attributes.get("value");
