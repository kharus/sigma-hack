...
public class DocGen {
     ...
     */
    protected Map relationsByKB = new HashMap();
    /**
    ...
    /**
     * A TreeMap of TreeMaps of Lists where the keys are
     ...
     */
    protected List<String> namespacePrefixes = new ArrayList<String>();
    /**
    ...
    /**
     * Returns an List of Strings extracted from the range
     * argument (arg2) of the first retrieved statement formed with
     * predicate.  If no statement can be retrieved, the List
     * will be empty.
     *
     * @param kb        The KB from which to retrieve a statement with predicate
     * @param predicate
     * @return An List of Strings, which could be empty.
     */
    ...
    protected static List<String> getClientOntologyNames() {
        ...
        List<String> clientOntologyNames = new ArrayList<String>();
        try {
    ...
    /**
     * @return a TreeMap of TreeMaps of Lists where the keys
     ...
     * formattedTerm2->term21,term22...term2N
     */
    protected TreeMap createAlphaList(KB kb) { // , Map stringMap
        /*
          long t1 = System.currentTimeMillis();
                    ...
                    if (isLegalForDisplay(StringUtil.w3cToKif(term))
                        ...
                        if (StringUtil.isNonEmptyString(formattedTerm)) {
                            ...
                            if ((alset != null) && alset.contains(firstLetter)) {
                                ...
                                List al = (List) map.get(formattedTerm);
                                if (al == null) {
                                    al = new ArrayList();
     ...
     * structure.
     *
     * @param alphaList a TreeMap of TreeMaps of Lists.  @see
     *                  createAlphaList()
     */
                    ...
                    if (hasSubComponents(kb, term)) {
                        ...
                        while (!accumulator.isEmpty()) {
                            ...
                            for (Iterator it = sharesComponentsWith.iterator(); it.hasNext(); ) {
                                ...
                                List nextAttrs = ((List) nextPair.get(0));
                                List nextElems = ((List) nextPair.get(1));
                        ...
                        List localAttrs = ((List) localPair.get(0));
                        List localElems = ((List) localPair.get(1));
     ...
     * displayed.
     *
     * @param alphaList a TreeMap of TreeMaps of Lists.
     */
     ...
     * @param ontology The name of the ontology that will be checked
     * @return An List of tokens (Strings) that are used as
        ...
        try {
            if (namespacePrefixes.isEmpty()) {
                synchronized (namespacePrefixes) {
                    ...
                    List<String> nsprefs = new ArrayList<String>();
    ...
    protected List getPredicates(KB kb, boolean requireNamespace) {
        ...
        List cached = null;
        try {
            cached = (List) getRelationsByKB().get(kb);
            if (cached == null) {
                ...
                if (relations != null && !relations.isEmpty()) {
                    ...
                    for (itr = relations.iterator(); itr.hasNext(); ) {
                        ...
                        if (!avoid.contains(relation)) {
                            List statements = kb.askWithPredicateSubsumption(relation,
                                    1,
                                    term);
                            // if (isFormula) {
        ...
        try {
            ...
            for (String pred : preds) {
                ...
                for (it = terms.iterator(); it.hasNext(); ) {
                    ...
                    attrs.addAll((List) newPair.get(0));
                    elems.addAll((List) newPair.get(1));
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        ...
        try {
            ...
            if (StringUtil.isNonEmptyString(className)
                ...
                String result = "<tr>" +
                        ...
                        "<a href=\"" +
                        kbHref +
                        StringUtil.toSafeNamespaceDelimiter(kbHref, className) +
                        suffix +
                        "\">" +
                        showTermName(kb, className, language) +
                        "</a>" +
                        "  </td>" +
                        StringUtil.getLineSeparator() +
                        "  <td></td><td></td><td></td>" +
    ...
    protected String generateTocHeader(KB kb, TreeMap alphaList, String allname) {
        ...
        if (StringUtil.emptyString(getTocHeader())) {
            StringBuilder result = new StringBuilder();
            try {
                List keyList = new ArrayList(alphaList.keySet());
    ...
    public void generateHTML(KB kb, String language, boolean simplified, String formatToken) {
        ...
        try {
            ...
            // A Map where the keys are the term names and the values
            // are "headwords" (with quotes removed).
            /*
              Map headwordMap = createHeadwordMap(kb);
            ...
            */
            ...
            // a TreeMap of TreeMaps of Lists.  @see createAlphaList()
            TreeMap alphaList = getAlphaList(kb); // headwordMap
            // Headword keys and List values (since the same headword can
    ...
    /**
     * Parses a file of delimited fields into an List of
     * Lists.  If rowFlags contains any Strings, the method
    ...
    protected List<List<String>> convertRowStringsToLists(List<String> rows, char delimiter) {
        ...
        try {
            ...
            for (int c = 0; c < rowCount; c++) {
                ...
                // row will be an empty List if line is an empty
                // String.
                result.add(row);
            }
    ...
    public class PresentationNameComparator implements Comparator {
        ...
        public boolean equals(Object obj) {
            ...
            if (ans) {
                ...
                KB eKB = getKB();
                DocGen dg = getDocGen();
                ans = ((pnc instanceof PresentationNameComparator)
