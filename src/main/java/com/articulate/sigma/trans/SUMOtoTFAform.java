...
public class SUMOtoTFAform {
    ...
    private static final Map<String, List<String>> signatures = null;
    public static KB kb;
    ...
    protected static List<String> relationExtractNonNumericSig(String rel) {
        ...
        int size = Integer.parseInt(rel.substring(rel.length() - 1));
     ...
     * returned List would give an empty string result for a relation.
     */
    public static List<String> relationExtractSigFromName(String rel) {
        ...
        String text = rel.substring(under + 2);
     ...
     * modified from its authored original
     ...
     private static String relationEmbedSig(String rel, List<String> sig) {
    ...
    /**
     ...
     private static void constrainVarsFromFunct(List<String> sig, String typeStr) {
     ...
     Set<String> types = varmap.get(arg);
     types.add(type);
        ...
        if (args.size() != 3) { // quant + list + formula
            ...
            if (args.get(1) != null) {
                ...
                for (String v : vars) {
                    ...
                    if (varmap.containsKey(v) && !StringUtil.emptyString(varmap.get(v))) {
                ...
                return "(" + opStr + "[" + varStr.substring(0, varStr.length() - 2) + "] : (" +
    ...
    private static String makePredFromArgTypes(Formula car, List<String> argTypeMap) {
        ...
        if (debug)
            System.out.println("SUMOtoTFAform.makePredFromArgTypes(): isEqualTypeOp(pred): " + isEqualTypeOp(pred));
        ...
        if (debug) System.out.println("SUMOtoTFAform.makePredFromArgTypes(): result: " + result + suffix);
    ...
    private static String processCompOp(Formula f, Formula car,
        ...
        if (debug)
            System.out.println("SUMOtoTFAform.processCompOp(): builtInNumericType(best): " + builtInNumericType(best));
        if (debug)
            System.out.println("SUMOtoTFAform.processCompOp(): rhs is term: " + rhs + " : " + Formula.isTerm(rhs.getFormula()));
        if (Formula.isTerm(rhs.getFormula())) {
            if (builtInNumericType(best)) {
                if (debug)
                    System.out.println("SUMOtoTFAform.processCompOp(): found constant: " + rhs + " with type " + best);
                numericConstantTypes.put(rhs.getFormula(), best);
            }
        }
        if (debug)
            System.out.println("SUMOtoTFAform.processCompOp(): lhs is term: " + lhs + " : " + Formula.isTerm(lhs.getFormula()));
        if (Formula.isTerm(lhs.getFormula())) {
            if (builtInNumericType(best)) {
                if (debug)
                    System.out.println("SUMOtoTFAform.processCompOp(): found constant: " + lhs + " with type " + best);
            ...
            if (car.getFormula().startsWith("instance")) {
                if (Formula.isTerm(s) && i == 1) {
                    if (args.size() > 1 && builtInOrSubNumericType(args.get(2))) {
                        if (debug)
                            System.out.println("SUMOtoTFAform.processOtherRelation(): found constant: " + s + " with type " + args.get(2));
    ...
    public static String numTypePromotion(Formula f, String parentType) {
        ...
        if (debug)
            System.out.println("SUMOtoTFAform.numTypePromotion(): isNumericType(parentType): " + isNumericType(parentType));
        if (findType(f) != null)
            if (debug)
                System.out.println("SUMOtoTFAform.numTypePromotion(): kb.compareTermDepth(findType(f),parentType): " + kb.compareTermDepth(findType(f), parentType));
    ...
    private static Map<String, Set<String>> cloneVarmap() {
        ...
        for (String s : varmap.keySet()) {
            ...
            newHashSet.addAll(varmap.get(s));
            newVarmap.put(s, newHashSet);
        }
        return newVarmap;
    }
        ...
        for (String s : args) {
            if (Formula.isVariable(s)) {
                ...
                if (kb.isRelation(s))
                    ...
                    Set<String> p = kb.immediateParents(s);
     ...
     * new ArrayList from two of equal size that has the most specific type
     * at each index.
     */
    ...
    private static String constrainOp(Formula f, String op, String parentType, List<String> args) {
        ...
        //String result = "(" + composed + " " + StringUtil.ListToSpacedString(newargs) + ")";
        String result = "(" + makePred + " " + StringUtil.ListToSpacedString(newargs) + ")";
    ...
    private static Formula constrainFunctVars(Formula f) {
        ...
        Map<String, Set<String>> oldVarmap = null;
        do {
            counter++;
    ...
    private static boolean typeConflict(Formula f, String type) {
        ...
        if (kb.kbCache.checkDisjoint(kb, type, rangeType)) {
            if (debug)
                System.out.println("SUMOtoTFAform.typeConflict(2): found type conflict between " + type + " and " + rangeType);
    ...
    private static boolean typeConflict(Set<String> types, String type) {
        ...
        for (String s : types) {
            if (kb.kbCache.checkDisjoint(kb, s, type)) {
                if (debug)
                    System.out.println("SUMOtoTFAform.typeConflict(2): found type conflict between " + s + " and " + type);
                errors.addAll(KBcache.errors);
                return true;
            }
        }
    ...
    public static String sortFromRelation(String rel) {
        ...
        String sigStr = sigBuf.substring(0, sigBuf.length() - 1);
    ...
    public static String process(Formula f, boolean query) {
        ...
        if (f != null && f.listP()) {
            ...
            for (String s : UqVars) {
                ...
                if (varmap.containsKey(s) && !StringUtil.emptyString(varmap.get(s))) {
    ...
    private static void buildNumericConstraints() {
        ...
        for (String t : intChildren) {
            List<Formula> intFormsTemp = kb.ask("ant", 0, t);
        ...
        for (String t : realChildren) {
            List<Formula> realFormsTemp = kb.ask("ant", 0, t);
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args == null) {
            ...
            if (args != null && args.length > 2 && args[0].equals("-c")) {
                ...
                if (debug)
                    System.out.println("SUMOtoTFAform.main(): contains ListFn__1Fn: " + kb.terms.contains("ListFn__1Fn"));
