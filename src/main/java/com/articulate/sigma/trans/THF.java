...
public class THF {
    ...
    private boolean containsUnknownTp(Map map) {
        ...
        if (debug)
            System.out.println("\n  THF.containsUnknownTp()  Enter containsUnknownTp with entries = " + entries);
        boolean found = false;
    ...
    public String oneKIF2THF(Formula form, boolean conjecture, KB kb) {
        ...
        while (containsUnknownTp(localsig)) {
            if (!oldsig.equals(localsig)) {
                ...
                f = new Formula();
                f.read(fsubst);
                THFdebugOut("\n  THF.KIF2THF()  Debug: f after is " + f);
                res = toTHF2(f);
                f = new Formula();
                f.read(kifFormula.toString());
                kifFormula = new StringBuilder();
    ...
    private boolean isBaseTp(String intype) {
        ...
        boolean res = intype.equals(unknownTp) || intype.equals(indTp);
        return res;
    }
    ...
    private List addStr(String str, List accu) {
        ...
        if (accu.isEmpty()) {
            reslist = Collections.singletonList(str);
        } else {
    ...
    private List toTHFList(String thfTp) {
        ...
        List res = null;
        List help = toTHFListH(thfTp, 0, List.of());
        if (!thfTp.startsWith("(")) {
            res = help;
        } else if (help.size() == 1) {
            res = List.of(help);
    ...
    private String toTHFTpList(List l) {
        ...
        for (int i = 1; i < l.size(); i++) {
            ...
            if (entry instanceof java.lang.String) {
                result.append(entry + typeDelimiter);
        ...
        /* the formula has form (h arg1 ... argN) */
            ...
            /* now h must be some non-logical symbol h with arguments arg1 ... argN */
                ...
                if (isBaseTp(headTpOld)) {
                    typeInfo = List.of(makeUnknownTp(len));
                } else {
                ...
                kifFormula.append(")");
                THFdebugOut("\n   Debug: declaring: " + headNew + " of type " + resType);
