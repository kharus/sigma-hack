...
public class WordNet implements Serializable {
    ...
    //public static Map<String,WordNet> wns = new HashMap<String,WordNet>();
    ...
    public Map<String, String> exceptionVerbPastHash = new HashMap<String, String>();
    // key root, value  is inflected (-ing) form
    public Map<String, String> exceptVerbProgHash = new HashMap<String, String>();
    /**
     * list of irregular plural forms where the key is the
     * plural, singular is the value.
     */
    public Map<String, String> exceptionNounHash = new HashMap<String, String>();
    // The reverse index of the above
    public Map<String, String> exceptionNounPluralHash = new HashMap<String, String>();
    /**
     ...
     */
    public Hashtable<String, List<AVPair>> relations = new Hashtable<String, List<AVPair>>();
    /**
    ...
    /**
     * a Map where the key is a 9-digit POS-prefixed sense and the value is a
     * the number of times that sense occurs in the Brown corpus.
     */
    public Map<String, Integer> senseFrequencies = new HashMap<String, Integer>();
    /**
     ...
     * synset, it's just the synset number.  Values are List(s) of String
     * verb frame numbers.
     */
    public Map<String, List<String>> verbFrames = new HashMap<String, List<String>>();
    /**
     ...
     * all words to lower case
     */
    public Map<String, List<String>> wordsToSenseKeys = new HashMap<String, List<String>>();
    public MultiWords multiWords = new MultiWords();
    /**
     * A Map with language name keys and Map<String,String> values.  The interior Map
    ...
    /**
     * Return an List of the string split by spaces.
     */
    ...
    /**
     * Return an List of the string split by periods.
     */
    ...
    /**
     * Write a Map of Maps where the key is a word sense of the
     * form word_POS_num signifying the word, part of speech and number
     * of the sense in WordNet.  The value is a Map of words and the
     * number of times that word cooccurs in sentences with the word sense
     * given in the key.
    ...
    public static void writeWordCoFrequencies(String fname, Map<String, Map<String, Integer>> senses) {
        ...
        try {
            ...
            for (String s : senses.keySet()) {
                Map<String, Integer> values = senses.get(s);
                if (values.size() > 0) {
    ...
    public static void testWordFreq() {
        ...
        // Map<String,TreeSet<AVPair>> wordFrequencies
    ...
    private void processPointers(String synset, String pointers) {
        ...
        while (m.lookingAt()) {
            ...
            List<AVPair> al = new ArrayList<AVPair>();
    ...
    public String getSUMOMapping(String synset) {
        ...
        switch (synset.charAt(0)) {
            case '1':
                return nounSUMOHash.get(synset.substring(1));
            case '2':
                return verbSUMOHash.get(synset.substring(1));
            case '3':
                return adjectiveSUMOHash.get(synset.substring(1));
            case '4':
                return adverbSUMOHash.get(synset.substring(1));
            case '5':
                return adjectiveSUMOHash.get(synset.substring(1));
        }
        ...
        try {
            ...
            while ((line = lr.readLine()) != null) {
                ...
                if (m.matches()) {
                    ...
                    } else if (line != null && line.length() > 0 && line.charAt(0) != ';') {
                        System.out.println("Error in WordNet.readNouns(): No match in "
                                + nounFile.getCanonicalPath() + " for line " + line);
                    }
                }
    ...
    /**
     * Merge a new HashSet of word co-occurrence statistics into the existing
     * set.
     */
    public void mergeWordCoFrequencies(Map<String, Map<String, Integer>> senses) {
        ...
        for (String s : senses.keySet()) {
            if (wordCoFrequencies.containsKey(s)) {
                Map<String, Integer> newValues = senses.get(s);
                Map<String, Integer> oldValues = wordCoFrequencies.get(s);
    ...
    public void readWordCoFrequencies() {
        ...
        wordCoFrequencies = new HashMap<String, Map<String, Integer>>();
        LineNumberReader lr = null;
        int counter = 0;
        File wfFile = null;
        ...
        try {
            ...
            while ((line = lr.readLine()) != null) {
                ...
                if (m.matches()) {
                    ...
                    List<String> al = wordsToSenseKeys.get(word);
                    if (al == null) {
                        al = new ArrayList<String>();
    ...
    /**
     * Read word sense frequencies into a Map of PriorityQueues
    ...
    public String sumoSentenceDisplay(String input, String context, String params) {
        ...
        try {
            ...
            for (String sentence : sentenceList) {
                List<String> synsetList = WSD.collectWordSenses(sentence); //returns an List of synsets
                ...
                //List<String> synsetList = splitToArrayList(synsetColl); //removes all punctuation and whitespace
                //List<String> wordList = splitToArrayList(wordColl);

                result.append(sumoSentimentDisplay(sentence)); //attaches sentiment display already in HTML format
                Map<String, Integer> conceptMap = DB.computeConceptSentiment(sentence); //returns Map of key=SUMO concept, value=sentiment score
                ...
                List<String> SUMOtermList = new ArrayList<String>();
                ...
                for (int j = 0; j < listLength; j++) {
                    ...
                    List<String> words = synsetsToWords.get(synset);
                    if (words != null && words.size() > 0)
                        word = words.get(0);
                    else
                        word = "";
                    SUMOterm = SUMOtermList.get(j);
                    if (nounDocumentationHash.containsKey(synset.substring(1)) && synset.charAt(0) == '1') {
                        documentation = nounDocumentationHash.get(synset.substring(1));
                    ...
                    } else if (verbDocumentationHash.containsKey(synset.substring(1)) && synset.charAt(0) == '2') {
                        documentation = verbDocumentationHash.get(synset.substring(1));
                        result.append("<a href=\"WordNet.jsp?synset=" + synset + "&" + params + "\">" + synset + "</a> ");
                        result.append(" " + "<a href=\"WordNet.jsp?simple=no" + "&" + params + "&word=" + word + "&POS=2" +
                                "\">" + word + "</a>" + "-  " + documentation + ".\n");
                    } else if (adjectiveDocumentationHash.containsKey(synset.substring(1)) && synset.charAt(0) == '3') {
                        documentation = adjectiveDocumentationHash.get(synset.substring(1));
                        result.append("<a href=\"WordNet.jsp?synset=" + synset + "&" + params + "\">" + synset + "</a> ");
                        result.append(" " + "<a href=\"WordNet.jsp?simple=no" + "&" + params + "&word=" + word + "&POS=3" +
                                "\">" + word + "</a>" + "-  " + documentation + ".\n");
                    } else if (adverbDocumentationHash.containsKey(synset.substring(1)) && synset.charAt(0) == '4') {
                        documentation = adverbDocumentationHash.get(synset.substring(1));
                        result.append("<a href=\"WordNet.jsp?synset=" + synset + "&" + params + "\">" + synset + "</a> ");
                    ...
                    if (SUMOterm == null)
                        ...
                        if (conceptMap.containsKey(SUMOterm.substring(2, SUMOterm.length() - 1))) { //SUMOterm is in form &%term=, so must remove &% and = to match to a key in conceptMap
                            Integer conceptScore = conceptMap.get(SUMOterm.substring(2, SUMOterm.length() - 1));
    ...
    public String sumoSentimentDisplay(String sentence) {
        ...
        try {
            ...
            result.append("<b>Overall Sentence Sentiment Score: </b>" + overallSentiment + "<br>");
            if (scoreMap.size() != 0)
                result.append("<b>Individual (un-disambiguated) Word Sentiment Scores: </b>" + scoreMap + "<br><br>");
            else
    ...
    public String sumoFileDisplay(String pathname, String counter, String params) {
        ...
        try {
            ...
            StringBuffer result = new StringBuffer();
            result.append("<b>Overall File Sentiment Score: </b>" + overallSentiment + "<br><br>");
    ...
    public boolean isHyponymRecurse(String synset, String hypo, List<String> visited) {
        ...
        //    public Hashtable<String,List<AVPair>> relations = new Hashtable();
        // ~ is hyponym
        ...
        if (synsetBlock != null) {
            ...
            while (it.hasNext()) {         // Split apart the SUMO concepts, and store them as an associative array.
                ...
                if (type.compareTo("noun") == 0) {
                    documentation = nounDocumentationHash.get(synset);
                    ...
                    sumoEquivalent = nounSUMOHash.get(synset);
                } else {
    ...
    public String nounRootForm(String mixedCase, String input) {
        ...
        if ((exceptionNounHash.containsKey(mixedCase)) ||
            ...
            if (exceptionNounHash.containsKey(mixedCase))
                result = exceptionNounHash.get(mixedCase);
            else
                result = exceptionNounHash.get(input);
        } else {
     ...
     * Get the SUMO term for the given root form word and part of speech.
     ...
     if (StringUtil.emptyString(word))
     return null;
     Set<String> synsetBlock = null;  // A String of synsets, which are 8 digit numbers, separated by spaces.
    ...
    public String verbRootForm(String mixedCase, String input) {
        ...
        if ((exceptionVerbHash.containsKey(mixedCase)) ||
            ...
            if (exceptionVerbHash.containsKey(mixedCase))
                result = exceptionVerbHash.get(mixedCase);
            else
                result = exceptionVerbHash.get(input);
        } else {
     ...
     *
     * @return an List of 9 digit synset Strings
     */
     ...
     * and values that are Lists of synset Strings
     */
    public TreeMap<String, List<String>> getSenseKeysFromWord(String word) {
        ...
        if (senseKeys != null) {
            for (int i = 0; i < senseKeys.size(); i++) {
                String senseKey = senseKeys.get(i);                // returns a word_POS_num
                ...
                if (senseKey != null) {
                    ...
                    if (POS != null) {
                        String synset = WordNetUtilities.posLettersToNumber(POS) + senseIndex.get(senseKey);
                        if (synset != null) {
                            List<String> words = synsetsToWords.get(synset);
                            if (words != null) {
                                Iterator<String> it2 = words.iterator();
                                while (it2.hasNext()) {
                                    String newword = it2.next();
                                    List<String> al = result.get(newword);
                                    if (al == null) {
                                        al = new ArrayList<String>();
    ...
    public String getDocumentation(String synset) {
        ...
        switch (POS) {
            case '1':
                return nounDocumentationHash.get(bareSynset);
            case '2':
                return verbDocumentationHash.get(bareSynset);
            case '3':
                return adjectiveDocumentationHash.get(bareSynset);
            case '4':
                return adverbDocumentationHash.get(bareSynset);
        }
        return null;
    }
    ...
    public String displaySynset(String sumokbname, String synset, String params) {
        ...
        switch (POS) {
            case '1':
                gloss = nounDocumentationHash.get(bareSynset);
                SUMOterm = nounSUMOHash.get(bareSynset);
                POSstring = "Noun";
                break;
            case '2':
                gloss = verbDocumentationHash.get(bareSynset);
                SUMOterm = verbSUMOHash.get(bareSynset);
                POSstring = "Verb";
                break;
            case '3':
                gloss = adjectiveDocumentationHash.get(bareSynset);
                SUMOterm = adjectiveSUMOHash.get(bareSynset);
                POSstring = "Adjective";
                break;
            case '4':
                gloss = adverbDocumentationHash.get(bareSynset);
                SUMOterm = adverbSUMOHash.get(bareSynset);
                POSstring = "Adverb";
                break;
        }
        if (gloss == null) {
        ...
        if (al3 != null) {
            ...
            while (it3.hasNext()) {
                ...
                List<String> al2 = synsetsToWords.get(avp.value);
                if (al2 != null)
                    words.addAll(al2);
                Iterator<String> it2 = words.iterator();
                while (it2.hasNext()) {
    ...
    public String displayByKey(String sumokbname, String key, String params) {
        ...
        String synset = senseIndex.get(key);
    ...
    private SimpleElement toXML() {
        ...
        while (it.hasNext()) {
            ...
            switch (synset.charAt(0)) {
                case '1':
                    gloss = nounDocumentationHash.get(synset.substring(1));
                    SUMO = nounSUMOHash.get(synset.substring(1));
                    break;
                case '2':
                    gloss = verbDocumentationHash.get(synset.substring(1));
                    SUMO = verbSUMOHash.get(synset.substring(1));
                    break;
                case '3':
                    gloss = adjectiveDocumentationHash.get(synset.substring(1));
                    SUMO = adjectiveSUMOHash.get(synset.substring(1));
                    break;
                case '4':
                    gloss = adverbDocumentationHash.get(synset.substring(1));
                    SUMO = adverbSUMOHash.get(synset.substring(1));
                    break;
            }
            if (gloss != null)
                gloss = gloss.replaceAll("\"", "&quote;");
            List<String> al2 = synsetsToWords.get(synset);
            ...
            List<AVPair> al = relations.get(synset);
            if (SUMO != "") {
    ...
    public String getTransitivity(String synset, String word) {
        ...
        for (int i = 0; i < frames.size(); i++) {
            int value = (Integer.valueOf(frames.get(i))).intValue();
    ...
    private void writeAdjectivesProlog(PrintWriter pw, KB kb) throws IOException {
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                String synset = it2.next();
                String sumoTerm = adjectiveSUMOHash.get(synset);
                if (sumoTerm != null && sumoTerm != "") {
                    String bareSumoTerm = WordNetUtilities.getBareSUMOTerm(sumoTerm);
                    bareSumoTerm = multipleMappingToProlog(bareSumoTerm);
    ...
    private void writeNounsProlog(PrintWriter pw, KB kb) throws IOException {
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                String synset = it2.next();
                String sumoTerm = nounSUMOHash.get(synset);
                if (sumoTerm != null && sumoTerm != "") {
                    String bareSumoTerm = WordNetUtilities.getBareSUMOTerm(sumoTerm);
                    ...
                    if (kb.childOf(bareSumoTerm, "Substance"))
                        countOrMass = "mass";
                    boolean instance = uppercase && mapping == '@';
                    if (mapping == '=') {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                while (it2.hasNext()) {
                    ...
                    String synset = senseIndex.get(senseKey);
    ...
    public void writeWordNetHyp() {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                List<AVPair> rels = relations.get(synset);
    ...
    public void writeWordNetG() {
        ...
        try {
            ...
            while (it.hasNext()) {
                String synset = it.next();
                String doc = nounDocumentationHash.get(synset);
            ...
            while (it.hasNext()) {
                String synset = it.next();
                String doc = verbDocumentationHash.get(synset);
                doc = processPrologString(doc);
            ...
            while (it.hasNext()) {
                String synset = it.next();
                String doc = adjectiveDocumentationHash.get(synset);
                doc = processPrologString(doc);
            ...
            while (it.hasNext()) {
                String synset = it.next();
                String doc = adverbDocumentationHash.get(synset);
                doc = processPrologString(doc);
    ...
    private String createNewSenseIndexKey(String base) {
        ...
        char num = '1';
        String key = base + num;
        while (senseIndex.containsKey(key) || reverseSenseIndex.containsValue(key)) {
            num++;
            key = base + num;
        }
        return key;
